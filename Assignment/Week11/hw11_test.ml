let%test _ = Hw11.interp (ParserMain.parse "if 0 < 1 then 2 else 3") Store.empty = Store.ClosureV ("y", Ast.Id "x", [("x", Store.NumV 3); ("true", Store.NumV 2)])
let%test _ = Hw11.interp (ParserMain.parse "if true then 2 else 3") Store.empty = Store.NumV 2
let%test _ = Hw11.interp (ParserMain.parse "if false then 0 else 1") Store.empty = Store.NumV 1
